...
def parse(conll_path_or_parsed_tuples: Union[List[List[tuple]], str], model_name:str = "./models/patb+cameltb-all_msa.model", logs: Dict[str, float] = None) -> List[List[tuple]]:
    st = time.time()
...
def parse_tuples(sentence_tuples: List[List[tuple]], model_name, logs: Dict[str, float] =  None) -> List[List[tuple]]:
...
def parse_conll(conll_path: str, model_name, logs: Dict[str, float] = None) -> List[List[tuple]]:
